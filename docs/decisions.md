...
## CONTENT-005 — DubrovnikFlow added as the Web Application Development proof point

The obvious reuse of the existing HomeStock pattern (`Card href` pointing the whole card at the proof-point page) didn't fit here: that pattern makes the *entire card* into one `<a>`, which works for HomeStock because its card deliberately gives up the standard Contact-deep-link behavior in favor of routing straight to `/homestock/`. The Web Application Development card was never meant to give up its own deep-link to `/kontakt/?service=web-app-development` — nothing in this task asked for that, and nesting a second `<a>` (to the external DubrovnikFlow site) inside an already-anchor-wrapped card isn't valid HTML regardless.
